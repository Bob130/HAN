...
from model import HAN as model_file
from data import hand_gesture_dataset as HandGestureDatasetFile
from data import FPHA_dataset as FPHADatasetFile
from data import DHG_dataset as DHGDatasetFile
from data import NTU_dataset as NTUDatasetFile
...
def main(args):
    ...
    if args.loop and not args.evaluate and not args.find_lr:
        checkpoint_dir = checkpoint_dir[0:-5]   # remove test/
    checkpoint_dir1 = '/'.join(checkpoint_dir.split('/')[:-2]) + '/'
    checkpoint_dir2 = '/'.join(checkpoint_dir.split('/')[-2:])

    global writer
    # writer = SummaryWriter(args.checkpoint + '_tensorboard')

    # create model
    model = model_file.HAN(num_classes=args.class_num, dp_rate=args.dpout_rate, input_frames=args.input_frames,
                           raw_input_dim=args.raw_input_dim, dataset=args.dataset, device=device)
    ...
    if args.dataset == 'HandGesture':
        ...
        train_loader = torch.utils.data.DataLoader(
            NTUDatasetFile.NTU_SKE(data_path='data/NTU_dir/xsub/train_data_joint.npy',
                                   label_path='data/NTU_dir/xsub/train_label.pkl', window_size=150, final_size=128,
                                   mode='train', random_choose=True, center_choose=False),
            batch_size=args.train_batch, shuffle=True, num_workers=args.workers)

        test_loader = torch.utils.data.DataLoader(
            NTUDatasetFile.NTU_SKE(data_path='data/NTU_dir/xsub/val_data_joint.npy',
                                   label_path='data/NTU_dir/xsub/val_label.pkl', window_size=150, final_size=128,
                                   mode='tset', random_choose=False, center_choose=True),
            batch_size=args.test_batch, shuffle=False, num_workers=args.workers)
    elif args.dataset == 'NTU-xview':
        train_loader = torch.utils.data.DataLoader(
            NTUDatasetFile.NTU_SKE(data_path='data/NTU_dir/xview/train_data_joint.npy',
                                   label_path='data/NTU_dir/xview/train_label.pkl', window_size=150, final_size=128,
                                   mode='train', random_choose=True, center_choose=False),
            batch_size=args.train_batch, shuffle=True, num_workers=args.workers)

        test_loader = torch.utils.data.DataLoader(
            NTUDatasetFile.NTU_SKE(data_path='data/NTU_dir/xview/val_data_joint.npy',
                                   label_path='data/NTU_dir/xview/val_label.pkl', window_size=150, final_size=128,
                                   mode='tset', random_choose=False, center_choose=True),
            batch_size=args.test_batch, shuffle=False, num_workers=args.workers)
    # --------------------------------------------------------------------------------------------------------------
    ...
    # finetune
    if args.finetune:
        print("\n\nFinetune dataset class num ", args.finetune_class_num)
        model.cls = nn.Sequential(
            nn.Dropout(args.dpout_rate),
            nn.Linear(128, args.finetune_class_num),
        )
        if torch.cuda.device_count() > 1:
            print("\n\nData parallel in ", torch.cuda.device_count(), "GPUs!")
            model = nn.DataParallel(model)
    ...
    if args.reduce_lr_on_plateau:
        scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=args.gamma, patience=args.patience,
                                                   threshold=0.0001, threshold_mode='abs', eps=1e-15)
    ...
    for epoch in range(args.start_epoch, args.epochs+1):  # loop over the dataset multiple times
        ...
        print('\nEpoch: %d | LR: %.1e | Best Train Acc: %.4f | Best Test Acc: %.4f' % (epoch, lr, best_train_acc, best_acc))
        ...
        best_train_acc = max(train_acces)
    ...
    logger.file.write('(BatchSize=%d)(Gpus=%d)\n'
                      ...
                      % (args.train_batch, torch.cuda.device_count(), args.lr, epoch, ','.join(map(str, reduce_lr_epochs)),
                         args.gamma, args.weight_decay, args.momentum, seed, best_train_acc, best_acc, best_epoch))
    # logger.file.write('\n' + argv_str + '\n')
    logger.close()
...
def train(train_loader, model, criterion, optimizer):
    ...
    for i, data in enumerate(train_loader):
        ...
        # plot progress
        bar.suffix = '({batch}/{size}) Data: {data:.3f}s |Batch: {bt:.3f}s |Total: {total:} |Loss: {loss:.4f} |Acc: {acc: .4f}'.format(
                    batch=i + 1,
...
def evaluate(val_loader, model, criterion):
    ...
    bar = Bar('Test ', max=len(val_loader))
    with torch.no_grad():       
...
if __name__ == "__main__":
    ...
    parser.add_argument('--finetune_class_num', default=45, type=int, metavar='N', help='Number of class 14 28  45 for finetuning')
