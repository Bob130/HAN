...
class HAN(nn.Module):
    def __init__(self, num_classes, dp_rate, input_frames, raw_input_dim, dataset, device):
        ...
        if dataset == 'FPHA':
            ...
            self.finger_list = [[0, 1],
                                ...
                                [14, 15, 16, 17],
                                [18, 19, 20, 21]]
            # finger index: 0:Wrist 1:thumb 2:index 3:middle 4:ring 5:pinky

        self.batch_norm2d_input = nn.BatchNorm2d(3)
        ...
        self.joint_att = ATT_Layer(input_size=128, output_size=128, h_num=h_num, h_dim=h_dim, dp_rate=dp_rate,
                                   domain="spatial", time_len=input_frames)

        self.finger_att = ATT_Layer(input_size=128, output_size=128, h_num=h_num, h_dim=h_dim, dp_rate=dp_rate,
                                    domain="temporal", time_len=input_frames)

        self.temporal_att = ATT_Layer(input_size=128, output_size=128, h_num=h_num, h_dim=h_dim, dp_rate=dp_rate,
                                      domain="temporal", time_len=input_frames)
        self.fusion_att = ATT_Layer(input_size=128, output_size=128, h_num=h_num, h_dim=h_dim, dp_rate=dp_rate,
                                    domain="temporal", time_len=input_frames)
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 3, 1, 2)
        x = self.batch_norm2d_input(x)
        x = x.permute(0, 2, 3, 1)

        # reshape x
        # x = x.reshape(-1, time_len * joint_num, self.raw_input_dim)

        finger_feature = [torch.mean(self.joint_att(self.input_map(x[:, :, finger, :])), -2)
        ...
        hand_feature = torch.stack(finger_feature, -2)
        hand_feature = torch.mean(self.finger_att(hand_feature), -2)

        finger_temporal_feature = [torch.mean(self.temporal_att(item), -2) for item in finger_feature]
        hand_temporal_feature = torch.mean(self.temporal_att(hand_feature), -2)

        finger_temporal_feature.append(hand_temporal_feature)
        temporal_feature = finger_temporal_feature
        temporal_feature = torch.stack(temporal_feature, -2)

        temporal_feature = torch.mean(self.fusion_att(temporal_feature), -2)

        pred = self.cls(temporal_feature)
        return pred
